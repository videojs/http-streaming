...
const runSimulation = function(options, done) {
  ...
  let sourceBuffer = player.vhs.mediaSource.sourceBuffers[0];
  ...
  results.effectiveBandwidth.push({
    time: 0,
    bandwidth: player.vhs.bandwidth
  });
  player.vhs.masterPlaylistController_.mainSegmentLoader_.segmentMetadataTrack_ = null;
  player.play();
    ...
    for (let j = 1; j <= intervalParams.timeTotal; j++) {
      ...
      if (segmentRequest) {
        ...
        if (segmentDownloaded > segmentSize) {
          if (!currentTime ||
               player.vhs.masterPlaylistController_.mainSegmentLoader_.mediaIndex !== null) {
